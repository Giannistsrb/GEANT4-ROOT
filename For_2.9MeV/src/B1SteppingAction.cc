...
void B1SteppingAction::UserSteppingAction(const G4Step *step)
{
  ...
  if(!fGe_nat)
  {
    fGe_nat = detectorConstruction->GetGenatVolume();
  }
  ...
  if(!fAulight)
  {
    fAulight = detectorConstruction->GetAulightVolume();
  }
...
//===================================================
 ...
 //============================================================================
  ...
  //--------------------------------------------------------------------------------
  //GeNat Foil:
  // Check if the particle is a neutron and if it reaches fGe_nat:
  if (volume != fGe_nat && postvolume == fGe_nat)
  {
    ...
    //Save the energy and mother volumes in text file:
    static std::ofstream stuff("NeutronsInsertGeNat.txt");
    static bool first = true;
    ...
    if (first)
  {
    ...
    fEventAction->GeNatNeutronEnergy(Energy); 

    //Energy comes from Molybdenum:
    if (vertexVolume == fMolybdenum){ fEventAction->GeNatNeutronEnergyfromMoly(Energy); }

    //Energy comes from TiT:
    if (vertexVolume == fTiT){ fEventAction->GeNatNeutronEnergyfromTiT(Energy); }

    //Energy comes from Copper:
    if (vertexVolume == fCopper){ fEventAction->GeNatNeutronEnergyfromCopper(Energy); }   

    //Energy comes from everywhere else except for Molybdenum and Copper:
    if (vertexVolume != fMolybdenum && vertexVolume != fCopper) { fEventAction->GeNatNeutronEnergyNotFromMoAndCu(Energy); }
  ...
  //--------------------------------------------------------------------------------
  //AuLight Foil:
  // Check if the particle is a neutron and if it reaches fAulight:
  if (volume != fAulight && postvolume == fAulight)
  {
    //Get the energy of the neutrons which reach AuLight foil:
    G4double Energy = 0.;
    Energy = step->GetPreStepPoint()->GetKineticEnergy(); 
    ...
    //Save the energy and mother volumes in text file:
    static std::ofstream stuff("NeutronsInsertAuLight.txt");
    static bool first = true;
    ...
    if (first)
  {
    ...
    //General energy (from everywhere):
    fEventAction->AuLightNeutronEnergy(Energy); 

    //Energy comes from Molybdenum:
    if (vertexVolume == fMolybdenum){ fEventAction->AuLightNeutronEnergyfromMoly(Energy); }

    //Energy comes from TiT:
    if (vertexVolume == fTiT){ fEventAction->AuLightNeutronEnergyfromTiT(Energy); }

    //Energy comes from Copper:
    if (vertexVolume == fCopper){ fEventAction->AuLightNeutronEnergyfromCopper(Energy); }   

    //Energy comes from everywhere else except for Molybdenum and Copper:
    if (vertexVolume != fMolybdenum && vertexVolume != fCopper) { fEventAction->AuLightNeutronEnergyNotFromMoAndCu(Energy); }
